...
const STRATEGY_COLORS = ['#C8102E', '#F5A623', '#B22222', '#2563EB']

const CustomTooltipBar = ({ active, payload, label }) => {
  if (!active || !payload?.length) return null
  return (
    <div className="bg-slate-800 text-white px-3 py-2 rounded-xl text-xs shadow-lg">
      <div className="font-700 mb-1">{PLATFORM_LABELS[label] ?? label}</div>
      {payload.map(p => (
        <div key={p.dataKey} className="flex items-center gap-2">
          <div className="w-2 h-2 rounded-full" style={{ background: p.fill }} />
          <span className="text-slate-300">{p.name}:</span>
          <span className="font-600">{fmtCompact(p.value)}</span>
        </div>
      ))}
    </div>
  )
        ...
        <div className="flex gap-1.5">
          {METRIC_OPTIONS.map(m => (
            <button
              ...
              className={`px-2.5 py-1 text-xs font-600 rounded-lg border transition-all ${
                metric === m.key
