  ...
  return (
    <div className="card">
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex items-center gap-1.5 bg-tyson-yellow/20 px-2.5 py-1 rounded-full">
          <span className="text-[11px] font-bold text-tyson-burgundy">SOV Tyson: {sov.toFixed(1)}%</span>
        </div>
      </div>
        ...
        <table className="w-full text-[13px]">
          <thead>
            <tr className="border-b border-slate-100">
              <th className="text-left py-3 pr-3 font-semibold text-slate-400 uppercase tracking-widest text-[11px]">Cuenta</th>
              <th className="text-right py-3 px-2 font-semibold text-slate-400 uppercase tracking-widest text-[11px]">Comunidad</th>
              <th className="text-right py-3 px-2 font-semibold text-slate-400 uppercase tracking-widest text-[11px]">Crecimiento</th>
              <th className="text-right py-3 px-2 font-semibold text-slate-400 uppercase tracking-widest text-[11px]">Posts</th>
              <th className="text-right py-3 pl-2 font-semibold text-slate-400 uppercase tracking-widest text-[11px]">ETR%</th>
            </tr>
          </thead>
          <tbody>
            {rows.map(row => (
              <tr
                key={row.id}
                className={`border-b border-slate-50 last:border-0 ${row.es_tyson ? 'bg-tyson-yellow/10' : ''}`}
              >
                <td className="py-3 pr-3">
                  <span className={`font-bold ${row.es_tyson ? 'text-tyson-burgundy' : 'text-slate-600'}`}>
                    {row.cuenta}
                  </span>
                </td>
                <td className="text-right py-3 px-2 font-semibold text-slate-700 tabular-nums">
                ...
                <td className="text-right py-3 px-2 text-slate-600 tabular-nums">{row.num_posts ?? '—'}</td>
                <td className="text-right py-3 pl-2 font-bold text-slate-700 tabular-nums">
                  {fmtPct(row.etr ?? 0, 2)}
                </td>
              </tr>
            ))}
  ...
  return (
    <div className="card border border-slate-100 card-hover">
      <div className="flex items-center gap-2 mb-3">
        ...
        <span className="text-sm font-bold text-slate-800">{row.cuenta}</span>
        <span className="text-[11px] text-slate-400 uppercase tracking-widest font-semibold">{PLATFORM_LABELS[row.plataforma]}</span>
      </div>
      {row.accion_mundial && (
        <div className="mb-3">
          <div className="text-[11px] font-semibold text-slate-400 uppercase tracking-widest mb-1.5">Acción Mundial</div>
          <p className="text-[13px] text-slate-600 leading-relaxed">{row.accion_mundial}</p>
        </div>
      )}
      {row.percepcion_audiencia && (
        <div className="mb-3">
          <div className="text-[11px] font-semibold text-slate-400 uppercase tracking-widest mb-1.5">Percepción</div>
          <p className="text-[13px] text-slate-600 leading-relaxed">{row.percepcion_audiencia}</p>
        </div>
      )}
      {row.ejemplo_url && (
        <a href={row.ejemplo_url} target="_blank" rel="noopener noreferrer"
          className="inline-flex items-center gap-1.5 text-[12px] text-tyson-burgundy font-bold hover:underline mt-2">
          Ver ejemplo <ExternalLink size={12} />
        </a>
      )}
    </div>
  )
